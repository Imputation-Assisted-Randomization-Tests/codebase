...
reject,p_values = iArt.test(G=median_imputer,Z=Z, X=X, Y=Y_adjusted, S=S, L=L, verbose=verbose, mode='cluster', thresholdForCovarianceImputation=thresholdForCovarianceImputation, random_state=random_state)
...
reject,p_values = iArt.test(Z=Z, X=X, Y=Y_adjusted, S=S, L=L, verbose=verbose, mode='cluster', thresholdForCovarianceImputation=thresholdForCovarianceImputation, random_state=random_state)
result_path = f"{folder_name}/test_ridge_{beta}.npy"
np.save(result_path, np.array([beta, reject,*p_values]))  # Adjust based on actual result structure

# Save the result for ridge regression with covariate adjustment
reject,p_values = iArt.test(Z=Z, X=X, Y=Y_adjusted, S=S, L=L, verbose=verbose, mode='cluster', thresholdForCovarianceImputation=thresholdForCovarianceImputation, covariate_adjustment=1, random_state=random_state)
...
reject,p_values = iArt.test(G=LightGBM, Z=Z, X=X, Y=Y_adjusted, S=S, L=L, verbose=verbose, mode='cluster', thresholdForCovarianceImputation=thresholdForCovarianceImputation, random_state=random_state)
result_path = f"{folder_name}/test_lightgbm_{beta}.npy"
np.save(result_path, np.array([beta, reject,*p_values]))  # Adjust based on actual result structure

# Save the result for LightGBM with covariate adjustment
reject,p_values = iArt.test(G=LightGBM, Z=Z, X=X, Y=Y_adjusted, S=S, L=L, verbose=verbose, mode='cluster', thresholdForCovarianceImputation=thresholdForCovarianceImputation, covariate_adjustment=3, random_state=random_state)
